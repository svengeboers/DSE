function [Cptot,CTtot] = fDarrieus3D3(c,lambda,B,Vinf)
...
theta_p = deg2rad(-10);       % Pitch angle [rad]

% Discretisation
H = 60;                             %Total height [m]
dZ = 3;                             %Section height [m]
Htower = 10;                         %Tower height [m]
M = H/dZ;                           %Number of sections
f = @(x) 15;%-(1/(2*H))*(x-H/2)^2+H/4;%15;%-(1/H)*(x-H/2)^2+H/4;      %Radius function, x = height, the function of the curvature of the blade
df = @(x) 0;%-(2/(2*H))*x+1/2;%0;%-(2/H)*x+1;      
...
for i = 1:1:M
    ...
    Vinf = Vinf;%Weibull2(htot);      % Incoming wind speed, from west to east [m/s]
